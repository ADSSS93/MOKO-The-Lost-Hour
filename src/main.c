...
static uint8_t shard_taken[4]={0,0,0,0}; static uint8_t puzzle_done[4]={0,0,0,0};
...
static void room_art(void){int r=16,g=18,b=32;if(room==1){r=26;g=15;b=34;}else if(room==2){r=12;g=28;b=29;}else if(room==3){r=30;g=22;b=12;}else if(room==4){r=25;g=8;b=10;}rect(0,GAME_TOP,320,182,r,g,b);rect(0,222,320,18,42,38,52);rect(35,105,72,7,65,58,78);rect(190,150,88,7,65,58,78);
 if(room<4&&!shard_taken[room]&&puzzle_done[room]){int sx=250-(room*35),sy=90+(room*28);rect(sx,sy,9,14,90,220,245);rect(sx+2,sy-3,5,3,180,250,255);} 
 if(room==0){rect(112,180,18,28,85,100,150);rect(116,185,10,8,180,200,230);} 
 if(room==1){rect(145,176,18,46,170,45,65);rect(220,195,18,27,170,45,65);rect(70,190,14,14,switch_a?60:150,switch_a?210:60,70);rect(270,110,14,14,switch_b?60:150,switch_b?210:60,70);} 
 if(room==2){rect(125,95,12,127,45,150,115);rect(260,130,12,92,45,150,115);rect(45,82,30,20,70,125,110);} 
 if(room==3){rect(90,190,45,6,215,130,35);rect(200,110,45,6,215,130,35);rect(150,84,22,22,180,115,35);} 
 if(room==4){rect(128,82,64,64,90,55,65);rect(144,98,32,32,205,185,120);} rect(px,py,12,18,220,190,120);rect(px+3,py+4,2,2,20,20,25);rect(px+8,py+4,2,2,20,20,25);}
static void interact(uint16_t now){if(!pressed(now,PAD_CROSS))return;
 if(room==0&&hit(px,py,12,18,105,170,32,45)){puzzle_done[0]=1;say(2);return;}
 if(room==1){if(hit(px,py,12,18,60,180,34,34))switch_a=1;if(hit(px,py,12,18,260,100,34,34))switch_b=1;if(switch_a&&switch_b&&!puzzle_done[1]){puzzle_done[1]=1;say(3);return;}}
 if(room==2&&hit(px,py,12,18,35,72,50,40)){if(shards>=2){puzzle_done[2]=1;say(4);}else say(7);return;}
 if(room==3&&hit(px,py,12,18,140,74,42,42)){if(shards>=3){puzzle_done[3]=1;say(5);}else say(7);return;}
 if(room==4&&hit(px,py,12,18,120,74,80,80)&&shards<4)say(6);
}
static void update_play(uint16_t now){int ox=px,oy=py;if(!(now&PAD_LEFT))px-=2;if(!(now&PAD_RIGHT))px+=2;if(!(now&PAD_UP))py-=2;if(!(now&PAD_DOWN))py+=2;if(px<0){if(room>0){room--;px=306;}else px=0;}if(px>308){if(room<4){room++;px=2;}else px=308;}if(py<GAME_TOP+4)py=GAME_TOP+4;if(py>204)py=204;
 if(room==1&&(hit(px,py,12,18,145,176,18,46)||hit(px,py,12,18,220,195,18,27))){px=ox;py=oy;}if(room==2&&(hit(px,py,12,18,125,95,12,127)||hit(px,py,12,18,260,130,12,92))){px=ox;py=oy;}
 interact(now);if(room<4&&!shard_taken[room]&&puzzle_done[room]){int sx=250-(room*35),sy=90+(room*28);if(hit(px,py,12,18,sx-4,sy-5,17,23)){shard_taken[room]=1;shards++;say(8);}}
 if(invuln>0)invuln--;if(room==3&&invuln==0&&hit(px,py,12,18,90,184,45,18)){health--;invuln=60;px=30;py=190;if(health<=0)state=STATE_GAMEOVER;}if(room==4&&shards==4&&hit(px,py,12,18,125,78,70,72))state=STATE_ENDING;if(pressed(now,PAD_START))state=STATE_PAUSE;if(timer_frames>0)timer_frames--;else state=STATE_GAMEOVER;}
