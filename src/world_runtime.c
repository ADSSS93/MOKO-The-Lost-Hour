...
int world_runtime_room_clear_reward(MokoWorldRuntime*w,int room){int reward;if(room<0||room>=5||w->room_clear_rewarded[room]||!enemies_room_cleared(&w->enemies,room))return 0;w->room_clear_rewarded[room]=1;w->combat_notice_room=room;w->combat_notice_timer=150;reward=100+room*25;if(bound_adventure){adventure_story_progress(bound_adventure,43,1);if(room==4)adventure_story_clear(bound_adventure,43);}return reward;}
