...
static const MokoEnemy*boss(const MokoWorldRuntime*w){int i;for(i=0;i<MOKO_ENEMY_COUNT;i++){const MokoEnemy*e=enemies_get(&w->enemies,i);if(e&&e->kind==ENEMY_HOUR_WARDEN)return e;}return 0;}
...
int world_runtime_boss_max_hp(const MokoWorldRuntime*w){(void)w;return 5;}
