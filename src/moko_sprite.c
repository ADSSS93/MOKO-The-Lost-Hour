...
static void moko_silhouette(int x,int y,int pulse){intro_tile(x+7,y,18,7,18,27,39);intro_tile(x+5,y+6,22,18,178,145,124);intro_tile(x+9,y+10,3,3,235,245,245);intro_tile(x+3,y+24,26,26,26,82+pulse*7,104+pulse*9);intro_tile(x-7,y+28,12,4,155,42,65);intro_tile(x+6,y+49,7,17,24,38,55);intro_tile(x+20,y+49,7,17,24,38,55);intro_tile(x+3,y+65,12,4,10,16,26);intro_tile(x+19,y+65,12,4,10,16,26);}
static void play_boot_intro(void){
    ...
    while(!intro_done(&in)){
        ...
        if(scene==INTRO_STUDIO){
            ...
            intro_tile(0,132,320,3,18,50,68);intro_tile(0,135,320,48,5,11,22);moko_silhouette(143,62,pulse);
...
void moko_sprite_draw(int x,int y,int facing,int walk_tick,int invuln,int anim_tick,uint32_t *ot,char **next_packet){
    ...
    spr=(SPRT *)(*next_packet);setSprt(spr);setXY0(spr,x,y-bob);setWH(spr,16,24);setUV0(spr,frame*16,0);setRGB0(spr,150,150,150);addPrim(ot,spr);*next_packet+=sizeof(SPRT);
