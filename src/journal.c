...
int journal_quest_cleared(const MokoQuests*q){int n,c=0;for(n=0;n<MOKO_QUEST_COUNT;n++)if(q->state[n]==QUEST_CLEAR)c++;return c;}
int journal_quest_active(const MokoQuests*q){int n,c=0;for(n=0;n<MOKO_QUEST_COUNT;n++)if(q->state[n]==QUEST_ACTIVE)c++;return c;}
int journal_quest_discovered(const MokoQuests*q){int n,c=0;for(n=0;n<MOKO_QUEST_COUNT;n++)if(q->state[n]!=QUEST_LOCKED)c++;return c;}
