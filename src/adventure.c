#include "adventure.h"
void adventure_reset(MokoAdventure*a){quests_reset(&a->quests);world_events_reset(&a->world);a->last_event=-1;a->last_reward=0;a->notice_timer=0;a->dash_count=0;world_events_enter_room(&a->world,0,&a->quests);}
void adventure_tick(MokoAdventure*a,int room,int shards,int echoes,int combo,int health,int focus){quests_tick(&a->quests,room,shards,echoes,combo,health,focus);if(a->notice_timer>0)a->notice_timer--;}
