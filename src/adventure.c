...
int adventure_interact(MokoAdventure*a,int room,int px,int py){int id=world_events_near(&a->world,room,px,py,24);int reward;if(id<0||!world_event_unlocked(&a->world,&a->quests,id))return 0;reward=world_events_interact(&a->world,&a->quests,room,px,py,24);if(!reward)return 0;a->last_event=id;a->last_reward=reward>1?reward:0;a->notice_timer=150;return reward;}
