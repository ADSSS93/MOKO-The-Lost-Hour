...
int finale_activate_socket(MokoFinale *f,int index,int shards){
 ...
 if(f->charge>=4){f->phase=FINALE_STABILIZE;f->phase_timer=FINALE_BASE_TIME;f->stability=40;f->sweep_tick=0;}
 return 1;
}
...
void finale_tick(MokoFinale *f,int dashing){
 ...
 if(f->phase==FINALE_STABILIZE){
  ...
  if(dashing){
   if(f->stability<100)f->stability+=3;
   if(f->stability>100)f->stability=100;
  }else if((f->sweep_tick%5)==0&&f->stability>0)f->stability--;

  if(f->stability<=0){
   f->stability=18;
   f->phase_timer+=FINALE_RECOVERY_TIME;
   f->perfect_chain=0;
  }

  if(f->phase_timer<=0){
   if(f->stability>=FINALE_CLEAR_STABILITY){
    f->phase=FINALE_COMPLETE;
    f->stability=100;
   }else{
    /* The player cannot simply wait out the finale: build stability with Memory Dash. */
    f->phase_timer=FINALE_RECOVERY_TIME;
    f->perfect_chain=0;
   }
  }
...
int finale_score_bonus(const MokoFinale *f){
 int b=f->charge*125+f->stability*5;
