...
# Camera state must exist before rect()/tri() so world primitives can be shifted
# while HUD/title/dialogue stay screen-space.
...
# Make TILE world drawing camera-aware.
...
# Make triangle world geometry camera-aware too.
...
# Extended world-space art for Silent Station. This adds a second concourse,
# maintenance bridge and terminal gate beyond x=320 so the camera has real space
# to traverse rather than merely panning over the old room.
...
art = r'''static void station_scroll_art(void){
    ...
    /* rails continue toward next district */
...
# Raised surfaces in the new half of Silent Station.
...
# Add matching visible platform geometry in world coordinates >320.
...
# World render mode wraps the actual room renderer only. HUD/dialogue remain fixed.
...
room_end = 'draw_moko();}\nstatic void collect_shard'
if room_end not in src:
    raise SystemExit('room_art end missing')
src = src.replace(room_end, 'draw_moko();camera_world_draw=0;}\nstatic void collect_shard', 1)

# Sprite rendering bypasses rect/tri, so apply camera offset explicitly.
...
# Expand Silent Station to 640 world units. Other rooms preserve legacy bounds.
...
# Update camera every gameplay frame after movement and room transitions.
...
# Visible marker for CI and emulator diagnosis.
