...
art = r'''static void hour_chamber_polish_art(void){
    ...
    if(room!=4)return;
    /* monumental clock face behind the existing sockets / Hour Warden */
    rect(109,66,102,5,102,54,67);rect(109,157,102,5,102,54,67);
    rect(105,75,5,78,88,46,59);rect(210,75,5,78,88,46,59);
    for(i=0;i<8;i++){
        int tx=116+(i%4)*29,ty=77+(i/4)*68;
        rect(tx,ty,8,3,183,125,72);
    }
    rect(158,92,4,48,124,73,83);rect(160,112,28,4,124,73,83);
    rect(158,111,5,5,232,190,101);
    /* floor perspective wedges make the arena read less like a flat room. */
    tri(30,207,96,165,115,207,42,23,31);tri(205,207,224,165,290,207,42,23,31);
    tri(118,207,160,169,202,207,55,28,37);
    /* one-shot recovery anchors: player can deliberately route through them. */
    for(i=0;i<3;i++){
        if(!chamber_anchor_used[i]){
            int glow=4+p;
            rect(ax[i]-glow,ay[i]-glow,10+glow*2,10+glow*2,43,27,58);
            rect(ax[i]-5,ay[i]-9,12,18,107,58,145);
            rect(ax[i]-2,ay[i]-5,6,8,225,177,91);
            rect(ax[i],ay[i]-3,1,5,46,38,48);rect(ax[i],ay[i],4,1,46,38,48);
        }else{
            rect(ax[i]-5,ay[i]+5,12,3,61,48,65);rect(ax[i]-1,ay[i],4,5,92,72,96);
        }
    }
    /* sweep warning markers: show where the existing finale hazards are headed. */
    if(finale_hazard_active(&finale)){
        int sx=finale_sweep_x(&finale),sy=finale_sweep_y(&finale);
        rect(sx-5,202,13,3,241,161,75);rect(sx-2,198,7,3,184,85,82);
        if(finale.phase==FINALE_STABILIZE){rect(25,sy-2,7,7,231,137,75);rect(288,sy-2,7,7,231,137,75);}
    }
    /* boss hit feedback is deliberately oversized for CRT/emulator readability. */
    if(finale.phase==FINALE_STABILIZE&&finale.boss_flash>0){
        int bx=finale_boss_x(&finale),by=finale_boss_y(&finale);
        rect(bx-9-p,by-9-p,38+p*2,38+p*2,116,52,141);
        rect(bx-4,by-4,28,28,232,166,91);
    }
}
'''
...
if room_old not in src:
    raise SystemExit('Hour Chamber render anchor missing')
...
logic = r'''static void hour_chamber_polish_tick(uint16_t n){
    ...
    if(pressed(n,PAD_CROSS))for(i=0;i<3;i++)if(!chamber_anchor_used[i]&&hit(px,py,12,18,ax[i]-14,ay[i]-14,38,34)){
        chamber_anchor_used[i]=1;chamber_anchors++;
        finale.stability+=20;if(finale.stability>100)finale.stability=100;
        finale.phase_timer+=90;score+=125;gameplay_reward(&gameplay,45);sfx(0x3100);
        break;
    }
}
'''
...
# Add final-arena objective feedback to the already-live HUD injection.
...
if hud_old not in src:
    raise SystemExit('Clockworks HUD anchor missing')
hud_new = hud_old + 'if(room==4&&finale.phase==FINALE_STABILIZE)FntPrint(font_id,"TIME ANCHORS %d/3  STABILITY %d%%\\n",chamber_anchors,finale.stability);'
...
if reset_anchor not in src:
    raise SystemExit('Clockworks reset anchor missing')
...
if marker not in src:
    raise SystemExit('Clockworks marker missing')
