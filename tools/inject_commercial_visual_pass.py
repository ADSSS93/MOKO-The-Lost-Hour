...
if not pat.search(src): raise SystemExit('commercial visual pass: station_art not found')
src = pat.sub(lambda _m: station + '\nstatic void street_art', src, count=1)
...
scroll = r'''static void station_scroll_art(void){
    ...
    for(i=0;i<5;i++){
        ...
        tri(x+18,82,x+33,82,x+25,70,52,49,66);rect(x+12,101,32,2,79,72,89);
    }
    tri(350,139,562,139,580,181,35,42,57);tri(350,139,580,181,344,181,24,31,45);tri(562,139,594,157,580,181,48,53,66);
...
if not pat.search(src): raise SystemExit('commercial visual pass: station_scroll_art not found')
src = pat.sub(lambda _m: scroll + '\nstatic void camera_tick', src, count=1)
...
if not pat.search(src): raise SystemExit('commercial visual pass: hud not found')
src = pat.sub(lambda _m: hud + '\nstatic void draw_journal', src, count=1)
