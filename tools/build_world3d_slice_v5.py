...
# Replace the authored hard camera zones with a damped spring plus only gentle composition bias.
# This keeps Moko moving inside the frame instead of making the world feel attached to him.
...
rep=r'''static void camera_follow(int px,int py,int facing){
    ...
    /* Commercial 2.5D staging: biases guide the eye but never freeze the player to a shot. */
    ...
    /* small dead zone removes micro-jitter from digital-pad direction changes */
    ...
    /* critically damped integer spring, cheap enough for PS1 */
...
# Replace the boar with a more readable hit/recovery silhouette. HP changes drive recoil in the real encounter.
...
# Replace the static gate with a real in-world completion animation. The two doors retract
# sideways after the encounter is cleared, making the mission ending readable without text.
...
rep=r'''static void gate(uint32_t*ot,char**pk,int clear,int tick){
    ...
    /* physical doors slide into the stone posts */
...
# Give Moko a stronger feline run silhouette: articulated forepaws counter-swing with hind legs.
# Existing v4 motion state, jump, landing squash, medallion and clock-hand tail stay intact.
...
# Foreground geometry creates intentional parallax/occlusion at the edges of the frame.
...
helper=r'''static void foreground_frame(uint32_t*ot,char**pk,int tick){
    ...
    /* near trees/stonework sit outside the playable corridor and occasionally crop at screen edge */
...
# Draw the near-frame geometry after the road but before characters so depth composition is intentional.
old='''road(ot,pk);\n    quad3g(ot,pk,6,'''
