...
# Add organic triangle primitive beside the legacy rectangle helper.
...
# Give Moko a softer grounded silhouette, idle breathing/bob and directional dash smear.
pat = re.compile(r'static void draw_moko\(void\)\{.*?\}static void draw_shard', re.S)
...
# Replace blocky NPC/enemy silhouettes with angular characters and grounded shadows.
pat = re.compile(r'static void living_art\(void\)\{.*?\}static void room_art', re.S)
rep = r'''static void living_art(void){
    ...
    for(i=0;i<MOKO_ENEMY_COUNT;i++){
        const MokoEnemy*e=enemies_get(&living.enemies,i);int flap;if(!e||!e->active||e->room!=room)continue;
...
# Add cinematic foreground silhouettes to break the flat horizontal layers.
...
# Draw organic foreground in all gameplay render paths, after world FX.
