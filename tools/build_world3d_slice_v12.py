...
src=src.replace('VILLAGE 3D V13 REV 303','VILLAGE 3D V16 REV 306 / VILLAGE 3D V15 REV 305 / VILLAGE 3D V14 REV 304 / VILLAGE 3D V13 REV 303',1)

# Runtime QA on run445 showed that any small near-camera box/prism can still
# explode into dark wedges on PS1 projection. Keep the foreground strictly as
# wide ground quads; readable props begin deeper in the authored village.
...
if n!=1: raise SystemExit('rev306 foreground replacement failed')
# Remove any surviving call to the older silhouette foreground from previous passes.
src=src.replace('foreground_frame(ot,pk,tick);','/* REV306 old foreground disabled */')
...
if anchor not in src: raise SystemExit('rev306 mesh scale anchor missing')
...
if old not in src: raise SystemExit('rev306 face anchor missing')
src=src.replace(old,new,1)

# Lower optical centre while keeping the near plane conservative.
...
src+='\n/* REV306 TOMBI-BENCHMARKED RUNTIME PASS: NO NEAR WEDGES / CLEAN VILLAGE VIEW */\n'
