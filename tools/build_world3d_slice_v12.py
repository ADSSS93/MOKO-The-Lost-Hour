...
road_rep=r'''static void road(uint32_t*ot,char**pk){
    ...
    /* authored walk lane: flat paving strips cannot explode at the near plane */
    for(i=0;i<6;i++){
...
# Safe foreground: no boxes/prisms before the mid-ground.
...
# Layered Village staging retained from REV308 but start solid scenery deeper.
...
helper=r'''static void commercial_village_v19(uint32_t*ot,char**pk,int tick){
    int i,p=(tick/10)&3;
    /* lane framing begins at z=1160, safely beyond the near clipping region */
    for(i=0;i<4;i++){
        int z=1160+i*120;
...
# Compact ears/paws and a moderate 1.25x mesh scale. Runtime #450 showed 1.5x
# was far too large and the extreme ear vertices crossed into a giant X.
...
# Keep face readable but proportional to the smaller hero.
...
# Back off the #450 zoom while keeping Moko comfortably readable.
